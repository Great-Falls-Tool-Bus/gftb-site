...
while IFS=' ' read -r key value remainder; do
  if [ "$key" = STABLE_BUILD_SOURCE_SHA ]; then
    if [ "$source_sha_seen" -ne 0 ] || [ -n "$remainder" ]; then
      echo "STABLE_BUILD_SOURCE_SHA must occur exactly once with one value" >&2
      exit 1
    fi
    source_sha="$value"
    source_sha_seen=1
...
if [ "$source_sha_seen" -ne 1 ] || [ "${#source_sha}" -ne 40 ]; then
  echo "STABLE_BUILD_SOURCE_SHA must be exactly 40 lowercase hex characters" >&2
  exit 1
fi
case "$source_sha" in
  *[!0-9a-f]*)
...
source_marker = rule(
    ...
    doc = "Writes the exact qualified source SHA from Bazel stable status.",
)
