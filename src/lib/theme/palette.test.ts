...
import { contrastRatio, formatRgb, parseCssColor } from '../../../scripts/lib/color-contrast.mjs';

// Every assertion here re-derives its numbers through the shared colour maths
// in scripts/lib/color-contrast.mjs (Ottosson OKLab + WCAG 2.1 relative
// luminance) so palette.ts, theme-gftb.css, and the meta spec's claims cannot
// drift apart silently — and so this suite and the acceptance contrast gates
// cannot disagree about what a ratio means. `parseCssColor` understands the
// `oklch(L% C Hdeg)` spelling palette.ts documents, `formatRgb` renders an
// opaque colour as the same `#rrggbb` form the palette pins, and
// `contrastRatio` is the WCAG definition over 8-bit sRGB.
	...
	it('round-trips all 77 oklch values to their documented hex', () => {
		for (const family of FAMILIES) {
			for (const shade of SHADES) {
				...
				expect(formatRgb(parseCssColor(token.oklch)), `${family}-${shade}`).toBe(token.hex);
			}
		}
	});
...
describe('WCAG claims (recomputed, not trusted)', () => {
	const bestInFamilyRatio = (family: Family, shade: Shade): number => {
		...
		return Math.max(contrastRatio(hex, palette[family][50].hex), contrastRatio(hex, palette[family][950].hex));
	};
	...
	it('every recorded ratio matches a fresh computation within 0.01', () => {
		for (const family of FAMILIES) {
			for (const shade of SHADES) {
				...
				expect(Math.abs(contrastRatio(token.hex, pole) - token.ratio), `${family}-${shade}`).toBeLessThan(0.01);
			}
		}
	});
	...
	it('the pinned light/dark anchors are AA on their grounds', () => {
		expect(contrastRatio(palette.primary[700].hex, palette.surface[50].hex)).toBeGreaterThanOrEqual(4.5);
		expect(contrastRatio(palette.primary[300].hex, palette.surface[950].hex)).toBeGreaterThanOrEqual(4.5);
	});

	it('body text holds: ink on paper and on cream stay >= 13', () => {
		expect(contrastRatio(palette.surface[950].hex, palette.surface[50].hex)).toBeGreaterThanOrEqual(13);
		expect(contrastRatio(palette.surface[950].hex, palette.surface[100].hex)).toBeGreaterThanOrEqual(13);
	});
	...
	it('exactly the disclosed families fail the 3:1 fill check on paper', () => {
		...
		const failing = accentFamilies.filter(
			(family) => contrastRatio(palette[family][500].hex, palette.surface[50].hex) < 3,
		);
