...
<Dialog
	...
	preventScroll
>
	<!-- Portal to <body>: Skeleton 4 dialogs do not auto-portal, and any ancestor
	     with a filter/backdrop-filter/transform becomes the containing block for
	     fixed descendants (CSS Filter Effects 2): a drawer rendered inline inside
	     a glass AppBar collapses to a strip pinned in the header. Portaling makes
	     `fixed inset-*` mean the viewport regardless of where the consumer mounts
	     this component. z tokens come from the semantic scale in app.css. -->
	<Portal>
