/**
 * Responsive image helper (TIN-2224 media pipeline consumer).
 *
 * Backfeed of MassageIthaca's proven srcset builder, kept brand-neutral for
 * any static spoke. Consumes the build-generated `static/image-manifest.json`
 * (produced by `scripts/optimize-images.js`) and turns a single image `src`
 * into ordered `srcset` strings per format, so components can ship right-sized
 * derivatives instead of multi-megabyte originals.
 *
 * This is intentionally thin: it does NOT invent sizes or paths. It only reads
 * the derivatives the optimize pipeline already emitted. If an image is missing
 * from the manifest, the raw `src` still works as the fallback, so a zero-photo
 * build (the committed empty manifest) degrades gracefully.
 */
 ...
 * without extension, e.g. "/photos/hero.jpg" -> "photos/hero".
 */
 ...
 *
 * @param src public path (e.g. "photos/hero.jpg" or "/photos/hero.jpg").
