...
test.describe('copy de-slop acceptance (restoration PR-5)', () => {
	...
	test('the front page carries no motion language and no em-dash', async ({ page }) => {
		await page.goto('/');
		expectCleanCopy(await collectUserFacingSurfaces(page), '/');
	});

	test('the 404 surface carries no motion language and no em-dash', async ({ page }) => {
		await page.goto('/definitely-not-a-page');
		expectCleanCopy(await collectUserFacingSurfaces(page), '/404');
	});
