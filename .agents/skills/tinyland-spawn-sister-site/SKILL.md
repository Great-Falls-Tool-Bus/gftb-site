...
- Do not add Cloudflare API credentials to the spoke. Edge mutation belongs to
  the declared infrastructure owner; application lifecycle belongs to the
...
- If an owner overlay will consume additional per-PR QA metadata, edit
  `.github/lanes.json` and run `just lanes-validate` before pushing. Install or
  change the actual lifecycle only in that product's owner overlay.
- If the spoke needs public client previews, require a reviewed owner-overlay
  lifecycle contract rather than inventing a sender in the spoke or recycling
  `alpha`/`beta`.
