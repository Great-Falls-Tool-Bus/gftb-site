...
modules into `tinyland-inc/bazel-registry`; infra repos such as Blahaj and lab
own only the substrate declared by their local manifests; tooling repos (this
scaffold, ci-templates) template and template-validate the others. Applying the wrong shape's rules — for example
...
2. **Run `just whoami`.** The recipe wraps `scripts/whoami.py`, which reads
   `tinyland.repo.json` and prints a one-screen summary with the matching
   skills to load. Schema-check the manifest with `just repo-manifest-validate`,
   which picks `docs/schemas/tinyland-repo-manifest.schema.json` or, when the
   manifest declares `schema_version: 2`,
