...
### Tag discipline: cache-first vs executor eligibility

The executor filter in `.bazelrc.flywheel`
(`--{build,test}_tag_filters=flywheel-eligible`) is the switch that decides
whether a target class may run on a remote executor. Give every packaging or
build target exactly one of these two disjoint tag shapes:

1. **Executor-safe**: hermetic and present in the GloriousFlywheel eligibility
   manifest as a proved class. Tag it `flywheel-eligible` (plus its class tag,
   e.g. `deployment-bundle-packaging`). It runs under both cache-first
   (`config=flywheel`) and executor-backed (`config=flywheel-executor`) modes.
   The `deployment_bundle` `pkg_tar` that packages `//:build` is the reference
   example.
2. **Cache-eligible but never executor**: tag it `gloriousflywheel-cache-only`
   plus `no-remote-exec` and deliberately OMIT `flywheel-eligible`. The executor
   tag-filter then excludes it while cache-first mode still serves a cached
   result for unchanged inputs. The reference example is the OPTIONAL adapter-node
   `container_image_context` `pkg_tar` target shipped in the scaffold
   `BUILD.bazel`: `container-image-and-push` is blocked at the GloriousFlywheel
   manifest layer (docs/CI-SCHEMA.md §4 and Operating Rule 8), so image assembly
   and push must never reach a remote executor even though the input set stays
   cacheable. That target is additive and inert under the adapter-static default
   (its container recipe inputs are absent) and only becomes meaningful once a
   spoke flips to adapter-node. `gloriousflywheel-cache-only` + `no-remote-exec`
   is the tag pair for that class; do not reuse it for anything an executor is
   allowed to run.
...
credential-helper state, or headers. The CI template, runner environment, or
operator shell supplies them through environment variables.
