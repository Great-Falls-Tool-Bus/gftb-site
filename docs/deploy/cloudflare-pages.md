...
A spoke **never** holds long-lived CF credentials in source. Secrets are
org/environment-provisioned, the workflow only publishes the built `build/`
artifact, and DNS/Access/Tunnel mutation remains with the declared
infrastructure authority.

## When to use it

- The spoke lives under a Cloudflare-fronted org domain with a declared edge
  and DNS authority.
...
jobs:
  build:
    ...
    steps:
      ...
      - uses: tinyland-inc/ci-templates/.github/actions/setup-nix@v2.12.2
      - name: Check static site
        run: nix develop --command just check-ci
      - name: Build static site
        run: nix develop --command just build-ci
