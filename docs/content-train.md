...
by their browser. On every full load of the home page the site opens with a
veil carrying the bus mark, holds it until the wiper stack has hydrated, and
then carries the visitor down to this section (`src/lib/intro`); any input
ends it at once, reduced motion and a URL fragment never start it, and it
never touches the rows or the manifest. Each row carries an "Edit" link to its own source file and
