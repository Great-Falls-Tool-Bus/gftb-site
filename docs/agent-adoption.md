...
| Lane metadata | `.github/lanes.json`, CI workflows | Build/QA metadata only; product owner overlays own PR lifecycle and apply. |
...
- Static spokes own Cloudflare mutation credentials or infer application apply
  authority from a generic substrate.
...
6. Keep lane declarations as metadata; bind any live application lifecycle to
   the consuming product's owner overlay rather than this scaffold.
