...
1. **The dynamic-spoke is a flagged adapter MODE inside site.scaffold, not a
   ...
   - `package.json` / `pnpm-lock.yaml`: carry both sanctioned third-party
     adapters in the frozen graph (`@sveltejs/adapter-node` is pinned
     `^5.5.3`, matching the `MassageIthaca` reference). Rebrand selects the
     adapter without mutating dependency authority.
   ...
   Each edit is atomic (`tmp` file then `mv`) and the sequence is recoverably
   idempotent: rerunning repairs a conversion interrupted between files. The
...
- The deployment owner must flip with the adapter: static publication remains
  an atomic-host concern, while a dynamic product requires its dedicated owner
  overlay for state, apply, health, rollback, and served readback. The removed
  shared-receiver design is historicized in
...
- **In-scope for this slice (implemented):** the `--adapter=node` flag, this ADR,
  the schema-role decision, and the AGENTS update. **Out of scope:** an
  application carrier, full adapter-node production build, Superforms, and any
