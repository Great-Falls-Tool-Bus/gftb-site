...
#
# Recoverably idempotent: every file is rewritten atomically and a rerun repairs
# a conversion interrupted between files.
...
# ─────────────────────────────────────────────────────────────────────
apply_adapter_node() {
  # The frozen graph carries both sanctioned third-party adapters. Rebrand only
  # selects which one the build uses, so it never mutates package.json without
  # regenerating pnpm-lock.yaml.
  if [[ -f package.json ]] \
    && ! jq -e '.devDependencies["@sveltejs/adapter-node"]' package.json >/dev/null; then
    echo "error: package.json must carry @sveltejs/adapter-node in the frozen graph" >&2
    return 1
  fi

  if [[ -f svelte.config.js ]] \
    && grep -q 'adapter-node' svelte.config.js \
    && grep -q ':node_modules/@sveltejs/adapter-node' BUILD.bazel \
    && [[ "$(jq -r '.taxonomy.spawned_repo_role' tinyland.repo.json)" == "app-stateful-spoke" ]]; then
    echo "adapter: already @sveltejs/adapter-node (idempotent no-op)"
    return 0
  fi

  # 1) svelte.config.js: deterministic adapter-node variant. Drops the
...
const config = {
	...
	kit: {
		...
		adapter: adapter({
			out: process.env.BUILD_OUTPUT_DIR ?? 'build',
		}),
		paths: {
...
SVELTE_NODE
  ...
  # 2) BUILD.bazel: select the adapter-node runfile from the frozen graph.
  if [[ -f BUILD.bazel ]]; then
    sed 's|:node_modules/@sveltejs/adapter-static|:node_modules/@sveltejs/adapter-node|g' \
      BUILD.bazel > BUILD.bazel.tmp && mv BUILD.bazel.tmp BUILD.bazel
  fi
  ...
  echo "  Bazel adapter runfile     -> @sveltejs/adapter-node"
...
# tofu/spoke.auto.tfvars — rewrite the generic infrastructure spoke slug
...
# Regenerate the committed page source map (src/lib/generated/source-map.json)
# from the just-rewritten tinyland.repo.json. The sed/jq passes above change
# repo.github (and thus the repoUrl baked into the committed map), so without
# this the map would still carry the hub URLs and fail `just source-map-check`
# on the first spawn. Idempotent and best-effort: only runs when node and the
# generator are present, so it never blocks a rebrand in a toolless environment.
if command -v node >/dev/null 2>&1 && [[ -f scripts/build-source-map.mjs ]]; then
  node scripts/build-source-map.mjs
  echo "source map: regenerated src/lib/generated/source-map.json for ${DOMAIN}"
fi
