...
// empty-object fallback carries zero-photo builds and fresh spawns) so
// callers can resolve the best candidate without re-scanning the
// filesystem. Each raster entry also records intrinsic width/height so
// consumers (src/lib/components/Picture.svelte) can reserve the layout box
...
// only when the decoder can rasterize them. Consumers (Picture.svelte) use
// these to set width/height + aspect-ratio and eliminate layout shift (CLS).
// Entries without a readable size ship without one and degrade gracefully.
